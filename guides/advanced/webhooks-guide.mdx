...
description: Receive and process Fanfare webhooks for real-time event notifications.
---
...
async function routeWebhookEvent(event: WebhookEvent) {
  const handlers: Record<
    string,
    (data: WebhookEvent["data"]) => Promise<void>
  > = {
    // Queue events
...
async function handleAdmissionCompleted(data: AdmissionCompletedData) {
  ...
  await db
    ...
    .where(
      and(
        eq(admissions.consumerId, consumerId),
        eq(admissions.experienceId, experienceId),
      ),
    );
