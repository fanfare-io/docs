...
description: Integrate Fanfare into single-page applications using React, Vue, or Angular.
---
...
export function ExperiencePage() {
  ...
  const { journey, state, status, error, start } = useExperienceJourney(
    "exp_your_experience_id",
    {
  ...
  return (
    <div className="journey-actions">
      ...
      {sequenceActions.includes("enter_draw") && (
        <button
          onClick={() => journey.perform("enter_draw")}
          className="primary-button"
        >
          Enter the Draw
        </button>
      ...
      {sequenceActions.includes("leave_participation") && (
        <button
          onClick={() => journey.perform("leave_participation")}
      ...
      {/* Show Queue Position */}
      {sequenceStage === "participating" &&
        context.participation?.type === "queue" && (
      ...
      {sequenceStage === "admitted" && (
        <AdmittedView
          admissionToken={context.admittanceToken}
          expiresAt={context.admittanceExpiresAt}
        />
      )}
    </div>
  );
...
export function QueuePosition({ participationId }: QueuePositionProps) {
  ...
  return (
    <div className="queue-position">
      ...
      <p className="hint">
        Stay on this page. We'll notify you when it's your turn.
      </p>
    </div>
  );
}
...
function ExperienceWidget() {
  ...
  return (
    <div>
      ...
      {actions.includes("enter_queue") && (
        <button onClick={() => journey?.perform("enter_queue")}>
          Join Queue
        </button>
      )}

      {snapshot?.sequenceStage === "admitted" && (
        <a href={`/checkout?token=${snapshot.context.admittanceToken}`}>
          Go to Checkout
        </a>
      )}
    </div>
