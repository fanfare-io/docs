...
export function JourneyActions({ view }: JourneyActionsProps) {
  ...
  return (
    <div className="journey-actions">
      {view.sequence.stage === "active" && "enter" in view.sequence && (
        <button onClick={() => view.sequence.enter()} className="primary-button">
          Join the Queue
        </button>
      )}

      {view.sequence.stage === "participating" && view.sequence.participation.type === "queue" && (
        <p className="hint">You are in the queue. Stay on this page for updates.</p>
      )}

      {view.sequence.stage === "admitted" && (
        <AdmittedView
          admissionToken={view.sequence.admissionGrant}
...
function ExperienceWidget() {
  ...
  return (
    <div>
      ...
      {view?.journeyStage === "routed" && view.sequence.stage === "active" && "enter" in view.sequence && (
        <button onClick={() => view.sequence.enter()}>Join Queue</button>
      )}

      {view?.journeyStage === "routed" && view.sequence.stage === "admitted" && (
        <button onClick={() => handleCheckout(view.sequence.admissionGrant)}>Go to Checkout</button>
      )}
    </div>
  );
}
...
1. **Local testing**: Use your development environment configuration from the SDK setup or dashboard
