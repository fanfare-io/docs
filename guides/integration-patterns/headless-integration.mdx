---
title: Headless Integration Guide
description: Integrate Fanfare using the core SDK without UI components for full control.
---
...
function updateUI(snapshot: JourneySnapshot) {
  ...
  switch (journeyStage) {
    ...
    case "needs_access_code":
      const isRequired = requirements.find(
        (r) => r.type === "access_code",
      )?.required;
...
function getAvailableActions(snapshot: JourneySnapshot) {
  const { journey: journeyActions, sequence: sequenceActions } =
