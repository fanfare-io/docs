...
Create a backend endpoint to generate exchange codes. For the full authorize/exchange contract — including request fields, response shapes, and identity consolidation via `canonicalSubject` — see the [External Authentication](/guides/authentication/external-authentication) guide.
  ...
  try {
    ...
    const response = await fetch(`${FANFARE_API_BASE_URL}/auth/external/authorize`, {
      ...
      headers: {
        ...
        "Authorization": `Bearer ${FANFARE_SECRET_KEY}`, // Server-side only!
      },
...
FANFARE_API_BASE_URL = "https://consumer.fanfare.io/api"
...
async def authorize_fanfare(user: User = Depends(get_current_user)):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{FANFARE_API_BASE_URL}/auth/external/authorize",
            json=payload,
            headers={
                "Content-Type": "application/json",
                "Authorization": f"Bearer {FANFARE_SECRET_KEY}",
            }
        )
...
const response = await fetch("/api/fanfare", {
  headers: {
    "Authorization": "Bearer sk_live_xxx", // NEVER expose a secret key in client code
  },
});
