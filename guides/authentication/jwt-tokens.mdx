---
title: JWT Tokens Guide
description: Implement server-side authentication using JWT tokens for secure backend-to-backend communication.
---
...
router.post("/validate-admission", async (req, res) => {
  try {
    const { queueId, drawId, consumerId, token } =
      req.body as ValidationRequest;
...
router.post("/checkout", async (req, res) => {
  const { admissionToken, consumerId, experienceType, distributionId, cart } =
...
async function validateAdmission(params: {
  type: string;
  distributionId: string;
  consumerId: string;
  token: string;
...
async function completeAdmission(params: {
  type: string;
  distributionId: string;
  consumerId: string;
}) {
...
async function validateAdmissionContext(
  token: string,
