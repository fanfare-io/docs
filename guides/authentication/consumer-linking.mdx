...
Link consumers via your own authentication system (server-side). For the full contract — including how Fanfare matches and consolidates external identities — see the [External Authentication](/guides/authentication/external-authentication) guide.
...
export async function POST(request: Request) {
  ...
  const response = await fetch("https://consumer.fanfare.io/api/auth/external/authorize", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "Authorization": `Bearer ${process.env.FANFARE_SECRET_KEY!}`, // Secret key!
    },
    body: JSON.stringify({
