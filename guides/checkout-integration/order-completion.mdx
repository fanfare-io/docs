...
export async function completeAdmission(params: CompleteAdmissionParams) {
  ...
  const response = await fetch(`${FANFARE_API_BASE_URL}${endpoint}`, {
    method: "POST",
    headers: {
...
export async function createOrder(params: CreateOrderParams) {
  const { consumerId, admissionToken, distributionId, distributionType, items, paymentId, totalAmount, currencyCode } =
    params;
