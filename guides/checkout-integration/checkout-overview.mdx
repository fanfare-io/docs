---
title: Checkout Integration Overview
description: Connect Fanfare experiences to your checkout flow, from admission to order completion.
---
...
function AdmittedState({
  ...
  return (
    <div className="admitted-state">
      ...
      <p className="warning">
        Don't close this page until you complete your purchase.
      </p>
    </div>
  );
}
...
app.post("/api/checkout", async (req, res) => {
  const { admissionToken, consumerId, distributionId, distributionType, cart } =
    req.body;
...
function EmbeddedCheckout({ admissionToken, onComplete }: Props) {
  ...
  return (
    <iframe
      ref={iframeRef}
      src={`/checkout/embedded?token=${admissionToken}`}
...
useEffect(() => {
  const handleBeforeUnload = (e: BeforeUnloadEvent) => {
    if (hasAdmissionToken && !orderComplete) {
      e.preventDefault();
      e.returnValue =
        "You have an active checkout session. Are you sure you want to leave?";
    }
  };
