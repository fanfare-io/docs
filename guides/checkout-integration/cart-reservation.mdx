---
title: Cart Reservation Guide
description: Hold inventory during checkout to prevent overselling when consumers are admitted.
---
...
async function onConsumerAdmitted(
  consumerId: string,
  productId: string,
  quantity: number,
) {
...
async function setupQueueInventory(
  queueId: string,
  productId: string,
...
async function handleAdmissionExpired(
  consumerId: string,
  experienceId: string,
) {
...
function CheckoutButton({ hasReservation, reservationExpired }: Props) {
  ...
  if (reservationExpired) {
    return (
      <button onClick={retryReservation}>
...
async function validateCheckoutReservations(
  consumerId: string,
  cart: CartItem[],
) {
  const reservations = await getActiveReservations(consumerId);
  ...
  const missing = cart.filter(
    (item) => !reservations.find((r) => r.productId === item.productId),
  );
