---
title: Custom Platform Integration Guide
description: Integrate Fanfare experiences with any e-commerce platform or custom system.
---
...
export function validateConfig() {
  ...
  const missing = required.filter(
    (key) => !fanfareConfig[key as keyof typeof fanfareConfig],
  );
...
<html>
  ...
  <body>
    <div class="product-details">
      ...
      <div
        id="fanfare-container"
        data-experience-id="exp_abc123"
...
(function () {
  ...
  function renderNotStarted(container, state) {
    ...
    if (startsAt) {
      ...
      container.innerHTML =
        '<div class="fanfare-not-started">' +
        "<p>Experience not yet available.</p>" +
        "</div>";
    }
  }
...
export function FanfareExperience({
  experienceId,
  productId,
  onAdmitted,
  onExpired,
}: FanfareExperienceProps) {
    ...
    case "waiting":
      return (
        <WaitingState
          ...
          estimatedWait={snapshot?.context?.estimatedWaitSeconds}
        />
      );
...
export async function validateFanfareAdmission(
  req: Request,
  res: Response,
  next: NextFunction,
) {
...
function verifySignature(payload: string, signature: string): boolean {
  const expected = crypto
    .createHmac("sha256", fanfareConfig.webhookSecret)
    .update(payload)
    .digest("hex");
...
```typescript
async function safeCompleteAdmission(
  params: CompleteAdmissionParams,
  maxRetries = 3,
) {
...
```typescript
async function completeAdmissionIdempotent(
  orderId: string,
  params: CompleteAdmissionParams,
) {
  ...
  const existing = await db.query.fanfareOrders.findFirst({
    where: and(
      eq(fanfareOrders.orderId, orderId),
      eq(fanfareOrders.fanfareCompleted, true),
    ),
  });
