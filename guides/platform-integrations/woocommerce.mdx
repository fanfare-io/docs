...
description: Integrate Fanfare experiences with your WooCommerce store on WordPress.
---
...
(function ($) {
  ...
  function updateStatusDisplay($container, state) {
    ...
    switch (state.stage) {
      case "not_started":
        if (state.startsAt) {
          statusHtml =
            '<div class="fanfare-countdown">Starts in: <span class="countdown-timer"></span></div>';
        }
        break;
      ...
      case "expired":
        statusHtml =
          '<div class="fanfare-expired">Your access has expired. Please try again.</div>';
        break;
    }
  ...
  function enableCheckout($container, productId) {
    ...
    if (displayMode === "replace") {
      ...
      var $checkoutBtn = $(
        '<button type="button" class="fanfare-checkout-btn button alt">' +
          "Complete Purchase" +
          "</button>",
      );
