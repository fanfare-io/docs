...
export function AppointmentBookingExperience({
  ...
  switch (view.sequence.stage) {
    ...
    case "denied":
      return <SessionExpiredState onRetry={view.sequence.reenter ?? start} />;
...
function SlotSelectionState({ sequence, service, availableSlots }: {
  sequence: Extract<SequenceView, { stage: "active"; distributionType: "appointment" }>;
  service: ServiceConfig;
  availableSlots: TimeSlot[];
}) {
...
function BookingConfirmedState({
  ...
  sequence: Extract<SequenceView, { stage: "participating"; participation: { type: "appointment" } }>;
  service: ServiceConfig;
}) {
