...
export function AuctionExperience({ auctionId, item }: AuctionExperienceProps) {
  ...
  const currentBid = context?.currentBid || context?.startingBid || "0.00";
  ...
  const bidIncrement = context?.bidIncrement || "50.00";
...
async function placeBid(journey: ExperienceJourney, amount: string) {
