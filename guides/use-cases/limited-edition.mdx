---
title: Limited Edition Drop Use Case
description: Distribute limited edition products fairly with draws and auctions.
---
    ...
    <section className="drop-details">
      ...
      <ol className="steps">
        <li>
          ...
          <p>
            Sign up with your email during the entry period. One entry per
            person.
          </p>
        </li>
        <li>
          ...
          <p>
            Winners will be selected randomly on{" "}
            {new Date(drawTime).toLocaleDateString()}.
          </p>
        </li>
        <li>
        ...
        <li>
          ...
          <p>
            You have 48 hours to complete your purchase at ${product.price}.
          </p>
        </li>
      </ol>
...
export function DrawExperience({
  ...
  const { journey, state, start } = useExperienceJourney(drawId, {
    autoStart: true,
  });
  ...
  return (
    <div className="draw-experience">
      ...
      {phase === "entry_closed" && (
        <EntryClosedState snapshot={snapshot} drawTime={drawTime} />
      )}
    ...
    <div className="entry-open-state">
      ...
      <div className="entry-stats">
        <span className="entries">
          {entryCount.toLocaleString()} entries so far
        </span>
      </div>
      ...
      <p className="terms">
        By entering, you agree to the draw rules and terms.
      </p>
    </div>
  );
}
      ...
      <div className="form-group checkbox">
        <label>
          <input
            ...
            required
          />
          I agree to the draw rules and terms of service
        </label>
      </div>
  ...
  return (
    <div className="entry-closed-state">
      ...
      {isEntered ? (
        ...
        <p className="missed">
          Entry is now closed. Follow us for future drops.
        </p>
      )}
    ...
    return (
      <div className="winner-state">
        ...
        <div className="claim-section">
          ...
          <button
            onClick={() =>
              (window.location.href = `/checkout/claim/${snapshot?.context?.claimToken}`)
            }
          >
            Claim Your Prize
          </button>
        ...
        <p className="warning">
          Don't miss out - unclaimed prizes go to the waitlist.
        </p>
      </div>
    );
  }
...
export function AuctionExperience({ auctionId, item }: AuctionExperienceProps) {
  const { journey, state, start } = useExperienceJourney(auctionId, {
    autoStart: true,
  });
    ...
    <div className="auction-experience">
      ...
      <div className="auction-main">
        <div className="current-bid-section">
          ...
          {context?.totalBids && (
            <span className="bid-count">{context.totalBids} bids</span>
          )}
        </div>
      ...
      <div className="quick-bids">
        {quickBidAmounts.map((amount) => (
          <button
            ...
            className={bidAmount === amount ? "selected" : ""}
          >
      ...
      <div className="custom-bid">
        ...
        <div className="bid-input-group">
          ...
          <input
            ...
            value={bidAmount}
            onChange={(e) =>
              setBidAmount(Math.max(minBid, Number(e.target.value)))
            }
            min={minBid}
            step={bidIncrement}
          />
        ...
        {bids.map((bid, index) => (
          <li key={bid.id} className={bid.bidderId === myId ? "my-bid" : ""}>
            <span className="bidder">
              {bid.bidderId === myId ? "You" : `Bidder ${bid.bidderNumber}`}
            </span>
...
const drawCommunicationSchedule = [
  ...
  {
    trigger: "24h_before_close",
    template: "draw-reminder",
    channels: ["email", "push"],
  },
  { trigger: "entry_closed", template: "draw-closed", channels: ["email"] },
  {
    trigger: "draw_complete_winner",
    template: "draw-winner",
    channels: ["email", "sms", "push"],
  },
  {
    trigger: "draw_complete_non_winner",
    template: "draw-non-winner",
    channels: ["email"],
  },
  {
    trigger: "24h_before_claim_deadline",
    template: "claim-reminder",
    channels: ["email", "sms"],
  },
  {
    trigger: "claim_expired",
    template: "claim-expired-waitlist",
    channels: ["email", "sms"],
  },
];
```
  ...
  return (
    <section className="draw-rules">
      ...
      <dl>
        ...
        <dt>Entry Period</dt>
        <dd>
          {new Date(draw.entryStart).toLocaleString()} -{" "}
          {new Date(draw.entryEnd).toLocaleString()}
        </dd>
        ...
        <dt>Selection</dt>
        <dd>
          Winners selected randomly using certified random number generator
        </dd>
