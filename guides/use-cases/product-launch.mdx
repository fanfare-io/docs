---
title: Product Launch Use Case
description: Manage high-demand product launches with virtual queues and controlled access.
---
...
function calculateQueueSettings(config: {
  ...
  const { totalInventory, expectedDemand, avgCheckoutTime, admissionWindow } =
    config;
  ...
  const totalLaunchDuration =
    (expectedDemand / concurrentAdmissions) * avgCheckoutTime;
...
async function enablePreQueue(queueId: string) {
  await adminClient.queues.update(queueId, {
    config: {
      ...
      preQueueMessage:
        "The launch begins at 10:00 AM ET. Stay on this page to secure your spot.",
    },
  });
}
```
...
export function ProductLaunchExperience({
  queueId,
  product,
}: ProductLaunchExperienceProps) {
  const { journey, state, start } = useExperienceJourney(queueId, {
    autoStart: true,
  });
  ...
  return (
    <div className="launch-experience">
      ...
      {stage === "waiting" && (
        <QueueState snapshot={snapshot} product={product} />
      )}

      {stage === "admitted" && (
        <AdmittedState snapshot={snapshot} product={product} />
      )}
...
function PreLaunchState({ snapshot }: { snapshot: JourneySnapshot }) {
  ...
  return (
    <div className="pre-launch-state">
      ...
      {preQueueMessage && (
        <p className="pre-queue-message">{preQueueMessage}</p>
      )}
      ...
      <div className="queue-stats">
        <div className="stat position">
          ...
          <span className="value">
            {position?.toLocaleString() || "Calculating..."}
          </span>
        </div>
  ...
  return (
    <div className="admitted-state">
      ...
      <p className="checkout-warning">
        Do not close this page until checkout is complete
      </p>
    </div>
  );
}
...
async function generateLaunchReport(queueId: string): Promise<LaunchReport> {
  ...
  return {
    ...
    dropoffAnalysis: {
      leftBeforeAdmission:
        analytics.totalEntered -
        analytics.totalAdmitted -
        analytics.currentlyWaiting,
...
const stageMessages = {
  ...
  waiting:
    "You're in line! Your position: {position}. Estimated wait: {time}. Do not close this page.",
