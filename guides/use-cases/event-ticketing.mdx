---
title: Event Ticketing Use Case
description: Manage ticket sales for high-demand events with fair access and controlled distribution.
---
    ...
    <div className="sold-out-state">
      ...
      <div className="alternatives">
        ...
        <ul>
          ...
          <li>
            <a href={`/events/${event.eventId}/resale`}>
...
export function TicketPurchaseExperience({
  experienceId,
  event,
  presaleCode,
}: TicketPurchaseExperienceProps) {
  ...
  return (
    <div className="ticket-selection">
      ...
      <div className="ticket-tiers">
        {event.ticketTiers.map((tier) => {
          const available =
            ticketAvailability.get(tier.tierId) ?? tier.quantity;
      ...
      {selectedTier && (
        <div className="quantity-selection">
          ...
          <div className="quantity-controls">
            ...
            <button
              onClick={() =>
                setQuantity(Math.min(event.maxTicketsPerOrder, quantity + 1))
              }
...
function PurchaseConfirmedState({
  snapshot,
  event,
}: {
  snapshot: JourneySnapshot;
    ...
    <div className="purchase-confirmed">
      ...
      <div className="next-steps">
        ...
        <ul>
          ...
          <li>
            Tickets will be available in your account 48 hours before the event
          </li>
      ...
      <div className="actions">
        <button onClick={() => (window.location.href = "/account/tickets")}>
          View My Tickets
        </button>
        <button onClick={() => addEventToCalendar(event)}>
          Add to Calendar
        </button>
      </div>
    </div>
...
export async function verifyTicketPurchase(verification: PurchaseVerification) {
  ...
  const existingOrders = await db.query.ticketOrders.findMany({
    where: and(
      eq(ticketOrders.consumerId, consumerId),
  ...
  if (
    totalTickets + orderDetails.quantity >
    event.maxTicketsPerOrder * event.maxOrdersPerCustomer
  ) {
...
export async function getTicketAvailability(
  ...
  await redis.setex(
    cacheKey,
    10,
    JSON.stringify(Object.fromEntries(availability)),
  );
...
export async function reserveTickets(
  ...
  return await db.transaction(async (tx) => {
    const tier = await tx
      ...
      .from(ticketTiers)
      .where(
        and(eq(ticketTiers.eventId, eventId), eq(ticketTiers.tierId, tierId)),
      )
      .for("update");
    ...
    const available =
      tier[0].quantity - tier[0].soldCount - tier[0].reservedCount;
...
async function handleSoldOut(data: QueueSoldOutEvent) {
  ...
  await adminClient.queues.notifyWaiting(experienceId, {
    type: "sold_out",
    message:
      "All tickets have been sold. You can join the waitlist for cancelled tickets.",
  });
  ...
  return (
    <section className="purchase-rules">
      ...
      <ul>
        ...
        {event.requireIdentification && (
          <li>Valid photo ID required at venue</li>
        )}
        {event.ticketsNonTransferable && <li>Tickets are non-transferable</li>}
        {event.requireBuyerAttendance && (
          <li>Purchaser must be present at entry</li>
        )}
      </ul>
    </section>
  );
