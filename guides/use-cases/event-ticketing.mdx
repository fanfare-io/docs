...
export function TicketPurchaseExperience({ experienceId, event, presaleCode }: TicketPurchaseExperienceProps) {
  ...
  if (view.sequence.stage === "participating") return <QueueState sequence={view.sequence} event={event} />;
  if (view.sequence.stage === "admitted") return <TicketSelectionState sequence={view.sequence} event={event} />;
  if (view.sequence.stage === "denied") return <SessionExpiredState onRetry={view.sequence.reenter ?? start} />;
    ...
    <div className="queue-state">
      ...
      <div className="queue-position">
        <div className="position-circle">
          <span className="position">{sequence.stage}</span>
...
function TicketSelectionState({
  ...
  sequence: Extract<JourneyView, { journeyStage: "routed" }>["sequence"] & { stage: "admitted" };
  event: EventConfig;
}) {
  const expiresAt = sequence.admissionGrantExpiresAt;
  ...
  const handleProceedToCheckout = () => {
    ...
    const orderData = {
      ...
      quantity,
      admissionGrant: sequence.admissionGrant,
    };
