...
- **Where to find it**: Your dashboard under **Settings → Developer → API keys**. For security, the full secret key is shown only once, right after you rotate it. Rotating issues a new key and retires the old one (it stops authenticating within seconds), so update your server when you rotate.
