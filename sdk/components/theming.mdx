...
One cascade detail is worth knowing before writing CSS against a themed widget. Layers 1 and 2 are stylesheet rules, so your own stylesheet outranks them — that is how overriding a token such as `--ff-widget-max-width` on `.fanfare-themed` works. Layer 3 is different: theme fields are emitted as *inline* custom properties, which outrank stylesheet rules. To change a field your theme already sets, change the theme rather than the CSS, or mark your rule `!important`.
