...
// Hoisted so the config keeps one identity for the life of the page. The provider keys its
// work on the values inside, so an inline literal is also safe — a constant is simply clearer.
const SHOPIFY_CONFIG = { proxyBaseUrl: "https://shop.myshopify.com/apps/fanfare/api" };

export function StorefrontProvider({ children }: { children: React.ReactNode }) {
  return (
    <FanfareShopifyProvider organizationId="org_abc123" publishableKey="pk_live_..." shopify={SHOPIFY_CONFIG}>
      {children}
...
`useFanfareExperience(product)` maps a Shopify product to the experience it gates. Pass a
`ShopifyProductRef` (`{ handle, id }`); it returns a discriminated union you switch over.
...
function ProductPage({ product }: { product: { handle: string; id: string } }) {
  const experience = useFanfareExperience({ handle: product.handle, id: product.id });

  switch (experience.status) {
    case "loading":
      return <Spinner />;
    case "gated":
      return <GatedPurchaseUI experienceId={experience.experienceId} />;
    case "ungated":
      return <NativePurchaseUI />;
    case "error":
      // Fail open: gating is unknown, so the native buy path renders and Shopify's checkout validation still enforces; a drop page may choose to hold instead.
      return <NativePurchaseUI />;
  }
}
```

`loading` — the answer is still being resolved. A synchronous source (`static`, `map`, or a
`metafield` source with the product preloaded) never produces it.

`gated` — the product carries a live experience; `experienceId` is the one it maps to.

`ungated` — the product carries no live experience; render your native purchase UI.

`error` — the answer could not be determined; `error.reason` is the typed cause. A failure is never
reported as `ungated`, and once a product has resolved, a later failure for the same product,
`proxyBaseUrl`, and override presence keeps the last result rather than flashing back to `loading`.

Pass `onResolutionError={(error) => report(error)}` to observe failed resolutions yourself;
`error.lookupKey` is the product handle or id and `error.reason` is one of `"network" | "http" |
"malformed" | "unavailable" | "override"`. Without it the SDK logs one `console.warn` per product and
reason to the developer console — nothing is reported to Fanfare. Supplying a handler replaces the
warn; the handler owns the signal.
